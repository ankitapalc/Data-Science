...
############ Data Cleaning ############
cleaned = txt_msg.replace(r'^.+@[^\.].*\.[a-z]{2,}$','email')
cleaned = cleaned.replace(r'^http\://[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(/\S*)?$','url')
cleaned = cleaned.replace(r'£|\$', 'currency')
cleaned = cleaned.replace(r'^\(?[\d]{3}\)?[\s-]?[\d]{3}[\s-]?[\d]{4}$','phonen')
cleaned = cleaned.replace(r'\d+(\.\d+)?', 'num')
cleaned = cleaned.replace(r'[^\w\d\s]', ' ')
cleaned = cleaned.replace(r'\s+', ' ')
cleaned = cleaned.replace(r'^\s+|\s+?$', '')
...
cleaned = cleaned.apply(lambda l: ' '.join(stm.stem(term) for term in l.split()))
